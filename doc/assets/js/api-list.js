YUI.add('api-list', (Y) =>
{
	...
	{
		const name = tab.get('label').toLowerCase();
	...
	{
		const queryType = filter.get('queryType');
	...
	{
		const frag         = Y.one(Y.config.doc.createDocumentFragment()),
	...
	{
		const currentTabIndex = tabs.selected.index;
