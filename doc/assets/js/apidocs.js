		...
		defaultRoute = Y.Pjax.defaultRoute.concat((req, res, next) => 
{
		...
		pjax.checkVisibility = function (tab)
		{
			...
			let panelNode = tab.get('panelNode'),
				visibleItems;
				...
				section.all('.index-item').each((itemNode) => 
{
		...
		{
			let hash      = win.location.hash.substring(1),
			...
			container.all('.linenums>li').each((lineNode, index) => 
{
		...
		{
			let terminators = /^(?:classes|files|elements|modules)$/,
				...
				setTimeout(() => 
{
					 ...
					 */
					let node = Y.one('#classdocs').getById(hash);
		...
		{
			let container = pjax.get('container');
		...
		{
			let status = res.ioResponse.status;
